...
func (d *genDriver) prepareDelete(req doc.DeleteRequestAny, a doc.Allocator) (deleteData, error) {
	item := req.ItemAny()
	if item == nil {
...
func (p *path) Handle(name string, value any) (string, any) {
	for i, node := range p.nodes {
		if node.domainName == name {
			if key, ok := genToBoltKey(value, node.ft); ok {
				node.value = key
				p.nodes[i] = node
			}
			break
...
func (p *path) BinaryAssignment(lhs string, rhs any) error {
	for i, node := range p.nodes {
		if node.boltName == lhs {
			if value, ok := genToBoltKey(rhs, node.ft); ok {
				node.value = value
				p.nodes[i] = node
			}
