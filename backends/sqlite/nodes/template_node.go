...
func getTemplateNodeState(key pipeline.Node, state *pipeline.State) *templateNodeState {
	ns := pipeline.GetNodeState[templateNodeState](key, state)
	if ns.mapped == nil {
		ns.mapped = make(map[string]string)
	}
	return ns
}

func (n *templateNode) Run(state *pipeline.State, input pipeline.RunInput) (*pipeline.RunOutput, error) {
	ns := getTemplateNodeState(n, state)
	...
	for _, pin := range input.Pins {
		...
		case *pipeline.ContentData:
			eb.AddError(n.runContentPin(ns, p, output))
		}
	}
	return output, eb.Err
}

func (n *templateNode) runContentPin(ns *templateNodeState, pin *pipeline.ContentData, output *pipeline.RunOutput) error {
	// XXX ideally everything is hooked up enough to check the format is "go"
	if pin.Name == "" {
		return fmt.Errorf("template node: missing pin name")
	}
	ic := newMakeTemplateContent(pin.Name, pin.Data, ns)
...
func (n *templateNode) processPrefix(c *makeTemplateContent) error {
	...
	c.nodeState.mapped[templatePrefixKey] = ""
	return nil
}
...
func (n *templateNode) processPackage(c *makeTemplateContent) error {
	...
	for i, line := range c.Lines {
		if strings.HasPrefix(line, prefix) {
			...
			c.nodeState.mapped[templatePackageKey] = ""
			return nil
		}
	}
...
func (n *templateNode) processAutogenerated(c *makeTemplateContent) error {
	...
	c.nodeState.mapped[templateUtilPackageKey] = ""
	c.nodeState.mapped[templateDatestampKey] = ""
	return nil
}
